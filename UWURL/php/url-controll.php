<?php
    ...
    if(!empty($full_url) && filter_var($full_url, FILTER_VALIDATE_URL)){
        ...
        if(mysqli_num_rows($sql) > 0){
            echo "Une erreur est apparue, veuillez réessayer!";
        }else{
            $sql2 = mysqli_query($conn, "INSERT INTO url (full_url, shorten_url, clicks, creator_id) 
                                         VALUES ('{$full_url}', '{$ran_url}', '0', '{$id}')");
            if($sql2){
        ...
        echo "$full_url - Ce n'est pas une URL valide!";
    }
?>